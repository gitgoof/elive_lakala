...
public class ScrollViewContainer extends RelativeLayout {
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (mMoveLen != 0) {
                if (state == AUTO_UP) {
                    LoginActivity.setSeprator(true);
                    mMoveLen -= SPEED;
                    if (mMoveLen <= -mViewHeight) {
                        mMoveLen = -mViewHeight;
                        state = DONE;
                        mCurrentViewIndex = 1;
                        LoginActivity.setViewStyle(false);
                    }
                } else if (state == AUTO_DOWN) {
                    LoginActivity.setSeprator(false);
                    mMoveLen += SPEED;
                    if (mMoveLen >= 0) {
                        mMoveLen = 0;
                        state = DONE;
                        mCurrentViewIndex = 0;
                        LoginActivity.setViewStyle(true);
                    }
                } else {
                    mTimer.cancel();
                }
...
//                LogUtil.d("ScrollViewContainer===", "mCurrentViewIndex:" + mCurrentViewIndex);
                if (mCurrentViewIndex == 0) {
                    bottomView.findViewById(R.id.iv_return).setVisibility(INVISIBLE);
                }
                vt.addMovement(ev);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!isMeasured) {
            ...
            if(Build.VERSION.SDK_INT>=24){
                mViewHeight+=UIUtils.getStatusHeight(this.getContext());
            }
            LogUtil.print(TAG,"SDK_INT:"+Build.VERSION.SDK_INT);
            LogUtil.print(TAG,"mViewHeight:"+mViewHeight+"   mViewWidth:"+mViewWidth+"   StatusHeight:"+UIUtils.getStatusHeight(this.getContext()));
    ...
    public OnTouchListener topViewTouchListener = new OnTouchListener() {
...
//                canPullUp = false;
    ...
    public OnTouchListener bottomViewTouchListener = new OnTouchListener() {
