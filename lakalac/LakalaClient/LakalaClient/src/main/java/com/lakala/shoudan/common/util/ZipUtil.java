...
public final class ZipUtil {
	...
	/**
	 * 解压一个zip文件到指定文件夹（如果zip中含有文件夹。则不保留zip文件夹内部的文件夹，直接解压文件里面的文件）
	 * @param zipFiString 	zip文件
	 * @param outPathString	解压目的文件目录
	 * @throws Exception
	 * @see ZipUtil#unZipFolder(String zipFileString, String outPathString)
	 */
	public static void unZipFolderWithNotSaveInternalFolder(String zipFiString,String outPathString) throws Exception{
		java.util.zip.ZipInputStream inZip = new java.util.zip.ZipInputStream(new java.io.FileInputStream(zipFiString));
		java.util.zip.ZipEntry zipEntry = null;
		String folderString = "";
		String szName = "";
		while ((zipEntry = inZip.getNextEntry()) != null) {
			szName = zipEntry.getName();
            LogUtil.print("szName>>>>>>>>", szName);
			if (zipEntry.isDirectory()) {
				folderString = szName;
			} else {
				szName = szName.replace(folderString, "");
                LogUtil.print("szName>>>>>>>>", szName);
				java.io.File file = new java.io.File(outPathString + java.io.File.separator + szName);
                LogUtil.print("zipUtil", file.getAbsolutePath());
				file.createNewFile();
				// get the output stream of the file
				java.io.FileOutputStream out = new java.io.FileOutputStream(file);
				int len;
