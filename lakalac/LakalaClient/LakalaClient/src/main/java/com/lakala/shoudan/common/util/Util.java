 ...
 * <p>
 * <br>1.日期格式化</br>
 * 2.文字ｔｏａｓｔ提示，隐藏显示键盘</br>
 * 3.金额，字符格式化，数值计算，字体大小设置</br>
 * 4.单位格转换-----文字单位，金额单位</br>
 * 5.校验字符合法性</br>
 * 6.软件及相关信息</br>
 * 7.设备硬件信息及网络信息</br>
 *
...
public class Util {
     ...
     *
     * @param dateString 毫秒字符串（纯数字字符串）
     * @param pattern    格式化格式  如：yyyy-MM-dd HH:mm ，yyyy MM dd HH:MM ,yyyy-MM-dd
     * @return 格式化后的日期字符串
     * @deprecated 使用DateUtil类中的方法
     */
     ...
     * 将cst格式化日期转换为毫秒格式时间
     *
     * @param CSTDateString cst格式的时间   例如：Tue May 21 14:32:31 CST 2013
     * @return
     */
    ...
    /**
     * @param num     数量
     * @param unit    单位 天/月/季度/年
     * @param pattern YYYY-MM-DD/YYYY年MM月DD日
     * @return 返回pattern相应的模式字符串
     */
    public static String addDate(String num, String unit, String pattern) {
        if ("日".equals(unit)) {
    ...
    /**
     * @param periodLen     周期个数
     * @param eachCycleDays 每个周期多少天 时间是两周，从今天开始算起
     * @return
     */
    public static String addDate(String systime, int periodLen, int eachCycleDays) {
        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date date = new Date();
        ...
        try {
            ...
            cal.setTime(date);
            cal.add(Calendar.DATE, eachCycleDays * periodLen - 1);
        } catch (ParseException e) {
            e.printStackTrace();
            date = new Date();
            cal.setTime(date);
    ...
    public static String formatBigDecimalAmount(String s) {
        String amount = "";
        try {
            BigDecimal bgAmount = new BigDecimal(s).setScale(2, BigDecimal.ROUND_HALF_UP);
     ...
     * 金额直接转换为double类型
     *
     * @param amount 金额是格式化后的,比如"123,456.00"
     * @return
     */
    ...
    public static String addSpaceToStringEnd(String string) {
        return addSpaceToStringEnd(string, 5);
    }

    /**
     * 字符串末尾加全角空格，对齐成指定数量个汉字
     *
     * @param string
     * @param len    指定对齐位数
     * @return
     */
    public static String addSpaceToStringEnd(String string, int len) {
        StringBuilder stringBuilder = new StringBuilder();
        int length = string.length();
        int appendCount = length < len ? len - length : 0;
...
//		}
    ...
    public static String formatFinanceAccout(String cardNumber, String bankName) {
        StringBuilder builder = new StringBuilder();
     ...
     * @return
     */
    public static String formatLicense1End1(String phoneNo) {
        if (TextUtils.isEmpty(phoneNo)) return "";

        int length = phoneNo.length();
        StringBuilder builder = new StringBuilder();
        builder.append(phoneNo.substring(0, 1));
        for (int i = 0; i < (length - 2); i++) {
            builder.append("*");
        }
        builder.append(phoneNo.substring(length - 1, length));
        return builder.toString();
    }
     ...
     * @param interval   分隔符
     */
    public static String formatCardNumberWith4Star(String cardNumber, String interval) {
        if (!checkCardNumber(cardNumber)) {
            return cardNumber;
        }
        int length = cardNumber.length();
    ...
    public static String yuan2Fen(String yuanStr) {
        ...
        String fenString = new BigDecimal(yuanStr).multiply(new BigDecimal("100")).setScale(0).toString();
        return fenString;
    }
    ...
    public static boolean max(String oneNumber, String anotherNumber) {
        ...
        int count01 = new BigDecimal(oneNumber).compareTo(new BigDecimal(anotherNumber));
        if (count01 == 1) {//大于
            return true;
        } else {
            return false;
...
//		sbBuilder.replace(6, 14, "********");//替换位区间左闭右开
    ...
    public static String formatIDCardNum(String cretifiNum) {
        if (null == cretifiNum || cretifiNum.length() == 0) {
            return null;
        }
        StringBuilder sbBuilder = new StringBuilder(cretifiNum);
...
//			sb.replace(1, index, str);
     ...
     * @return
     */
    public static boolean isAmountVaild(Context context, String amountString) {
        if (amountString != null && amountString.length() > 0) {
            ...
            try {
                ...
                LogUtil.print(e);
                ToastUtil.toast(context, R.string.toast_money_format_error);
            }
            // 金额必须大于0
            if (amount > 0.001) {
                return true;
     ...
     * 判断金额是否有效
     *
     * @param amountString 输入的金额
     * @param max          该业务限制的最大金额
     * @return true，false
     */
    public static boolean isAmountVaild(Context context, String amountString, double max) {
        if (isAmountVaild(context, amountString)) {
            try {
...
//		String regEx = "^[\\w\\d]+@[\\w\\d]+(\\.[\\w\\d]+)+$";
    ...
    public static boolean checkIdCard(String idCard) {
        ...
        if (matcherObj.matches()) {
            ...
            int sigma = 0;
            //系统数表
            Integer[] coeTable = {7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2};
            //校验码表
            String[] codeTable = {"1", "0", "X", "9", "8", "7", "6", "5", "4", "3", "2"};
    ...
    public static String getAppVersionCode() {
        ...
        try {
            ...
            PackageInfo pi = pm.getPackageInfo(ApplicationEx.getInstance().getPackageName(), 0);
            int ver = (pi.versionCode - 32);

            versionCode = "skb_" + (ver < 10 ? ("0" + ver) : ("" + ver));//从33版本开始计算的 故p_v 是 当前版本减去32
        } catch (Exception e) {
        }
        return versionCode;
    }
    ...
    public static String getChanel() {
        String CHANNELID = "0";
        try {
            ApplicationInfo ai = ApplicationEx.getInstance().getPackageManager().getApplicationInfo(
                    ApplicationEx.getInstance().getPackageName(), PackageManager.GET_META_DATA);
     ...
     * TextView 文字高亮显示
     *
     * @param color       需要显示高亮的颜色
     * @param str         需要高亮显示的字符串
     * @param from        起始位置
     * @param to          结束为止
     * @param isUnderline 是否要下划线
     * @return
     */
    public static SpannableStringBuilder buildColor(int color, String str, int from, int to, boolean isUnderline) {
        SpannableStringBuilder style = new SpannableStringBuilder(str);
        style.setSpan(new ForegroundColorSpan(color), from, to, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        if (isUnderline) {
            style.setSpan(new UnderlineSpan(), from, to, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        return style;
    }
...
//            	((WebView)view).getSettings().setDisplayZoomControls(false);
     ...
     *
     * @param activity
     * @param channelCode 第三方渠道号
     * @param callbackUrl 第三方回调地址
     * @see #splitCallBack(String, String)
     */
    public static void interceptBackEvent(Activity activity, String channelCode, String callbackUrl) {
     ...
     * @return
     */
    public static String addMarkToStringAfter(String string, String mark, int len) {
        StringBuilder stringBuilder = new StringBuilder();
        int length = string.length();
        int appendCount = length < len ? len - length : 0;
        stringBuilder.append(string);
    ...
    public static boolean isNewVersion() {
        ...
        if (osVersion.startsWith(UniqueKey.STABLETAG)) { //稳定版
            ...
            String replaceTarget = UniqueKey.STABLEVERSION.replace(UniqueKey.STABLETAG, "");
    ...
    public static String addAmount(String amt1, String amt2) {
        ...
        return formatAmount(String.valueOf(Double.parseDouble(amt1) + Double.parseDouble(amt2)));
    ...
    /**
     * 商户名称：商户名仅支持中文、字母、数字及半角括号、半角逗号和半角句号。
     *
     * @param merchantName 是否需要中文的\\。\\，\\（）
     * @return
     */
    public static boolean checkMerchantName(String merchantName) {
        Pattern p = Pattern.compile("[0-9\\w\\.\\,\\()\u4e00-\u9fa5]+");
     ...
     * @return
     */
    public static boolean checkNumAnd(String merchantName) {
    ...
    /**
     * 替你还
     * 生成订单号，由头字母+日期时间（到秒）+随机码组成，
     * 多媒体头字母为T，收款宝字母为S，如：S2014 0612 11 36 54 123456。
     * 首次产生订单号，后面其它步骤的订单号与其一致
     *
     * @return
     */
    public static String getLoanRandom() {
        ...
        int length = 6;//随机码6位
        for (int i = 0; i < length; i++) {
            int ranNum = (int) (Math.random() * 10);//[0-1)*10
            orderno.append(ranNum);
        }
    ...
    /**
     * @param dateValue 20141126142334
     * @param fromPar
     * @param toPar     MM-dd
     * @return
     */
    public static String formateDateSpecial(String dateValue, String fromPar, String toPar) {
        try {
            SimpleDateFormat df = new SimpleDateFormat(fromPar);
            Date date = df.parse(dateValue);
            SimpleDateFormat dfto = new SimpleDateFormat(toPar);
...
//			editText.setSelection(result.length());
    ...
    public static String getAppHash(Context context) {
        ...
        try {
            ...
            while ((byteCode = fileInputStream.read(bytes)) > 0) {
                messageDigest.update(bytes, 0, byteCode);
            }
            BigInteger bigInteger = new BigInteger(1, messageDigest.digest());
            appHash = bigInteger.toString(16);
