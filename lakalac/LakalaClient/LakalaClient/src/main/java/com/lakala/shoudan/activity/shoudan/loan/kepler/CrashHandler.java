...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	private String saveCrashInfoToFile(Throwable ex) {
		...
		try {
			...
			String fileName = path + "crash-" + DateUtils.getNowDate("yyyy-MM-dd HH-mm-ss") + CRASH_REPORTER_EXTENSION;
			...
			LogUtil.e(TAG, "an error occured while writing report file..." + e.toString());
		}
		return null;
	}
	...
	public void collectionDumpHprofDataIfOOM(Throwable ex) {
		...
		if (isOOM(ex)) {
			try {
				...
				LogUtil.e(TAG, "couldn’t dump hprof,  an error occurs while opening or writing files.");
			} catch (UnsupportedOperationException e) {
				LogUtil.e(TAG, "couldn’t dump hprof,  the VM was built without HPROF support.");
			}
		}
	}
