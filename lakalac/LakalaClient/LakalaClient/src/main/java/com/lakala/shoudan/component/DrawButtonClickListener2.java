...
import com.lakala.shoudan.activity.quickArrive.OneDayLoanApply2Activity;
import com.lakala.shoudan.activity.quickArrive.OneDayLoanApplyActivity;
import com.lakala.shoudan.activity.quickArrive.QuickArriveActivity2;
 ...
 * "立即提款"按钮点击事件
 */
public class DrawButtonClickListener2 implements View.OnClickListener {
    ...
    public DrawButtonClickListener2(final AppBaseActivity act) {
        mAct = act;
...
//                act.startActivity(intent1);
                mAct.startActivity(new Intent(mAct, OneDayLoanApply2Activity.class));
                PublicToEvent.MerchantlEvent( ShoudanStatisticManager.Merchant_jsType, act);
            }
        };
    ...
    public DrawButtonClickListener2 setStatistic(String statistic) {
                ...
                if(resultServices.isRetCodeSuccess()){
                    try {
                        ...
                        if (TextUtils.equals(firstUpgradeStatus, UpgradeStatus.NONE.name())
                            ...
                            if(TextUtils.equals(firstUpgradeStatus, UpgradeStatus.NONE.name())){
                                resultServices.retMsg = "您的资料不完整，快点击“确定”去提交资料吧!";
                            }else {
    ...
    private void getT0Status(){
        ServiceResultCallback callback = new ServiceResultCallback() {
            ...
            public void onSuccess(ResultServices resultServices) {
                ...
                else if(TextUtils.equals(resultServices.retCode,"005089")){
                    //未开通一日贷&&商户入网<30天
                    mAct.hideProgressDialog();
                    DialogCreator.createFullContentDialog(
                            mAct,"稍后再说", "立即申办", "您暂不满足开通“立即提款”业务的条件，建议申办“一日贷”",
                            new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialog, int which) {
                                    dialog.dismiss();
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    //标记是点击立即提款申办进入
                                    PublicEnum.Business.setWithdrawal(true);
    ...
    private void dealResult(final T0Status status) {
        ...
        {
            if (!(status == T0Status.COMPLETED||status==T0Status.ONEDAYLOAN)) {
                mAct.hideProgressDialog();
            }
            switch (status) {
                case COMPLETED: {
                    getDrawEnable(false);
//                    mAct.startActivity(new Intent(mAct, QuickArriveActivity2.class));
                    break;
                }
                case ONEDAYLOAN:
//                    getDrawEnable(true);
...
//                    ).show();
                    DialogCreator.createConfirmDialog(
                            mAct, "确定", status.getErrMsg()
                    ).show();
                    break;
                }
                case PROCESSING: {
                    DialogCreator.createConfirmDialog(
                            mAct, "确定", "系统正在处理您的立即提款开通申请，稍后会信息通知您审核结果"
                    ).show();
                    break;
                }
                default: {
...
//                        if (drawEnable) {
                            Intent intent2 = new Intent(mAct, QuickArriveActivity2.class);
