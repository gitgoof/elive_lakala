...
public class MonthView extends LinearLayout {
    ...
    private static String[] weekTitles = { "日", "一", "二", "三", "四", "五", "六" };

    public static MonthView create(ViewGroup parent, LayoutInflater inflater,
                                   DateFormat weekdayNameFormat, Listener listener, Calendar today) {
        final MonthView view = (MonthView) inflater.inflate(R.layout.l_month,
                parent, false);
        ...
        for (int offset = 0; offset < 7; offset++) {
            ...
            // 只用一个字显示星期，比如“周一”用“一”代替
            textView.setText(weekTitles[today.get(Calendar.DAY_OF_WEEK)-1]);
        }
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (i < numRows) {
                ...
                for (int c = 0; c < week.size(); c++) {
                    ...
                    String dayValue = Integer.toString(cell.getValue());

                    if (cell.isCurrentMonth()){
                        cellView.setText(dayValue);
                    }else {
                        cellView.setText("");
                    }
