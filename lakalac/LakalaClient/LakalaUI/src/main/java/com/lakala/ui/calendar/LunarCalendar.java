...
public class LunarCalendar implements Serializable {
    ...
    public static void main(String[] args) {
        ...
        clr.set(2014, 9, 1);
        LunarCalendar lunar = new LunarCalendar(clr.getTimeInMillis());
        ...
        System.out.println(LunarCalendar.Animals[lunar.getDeqiD()] + "日冲" + LunarCalendar.Animals[(lunar.getDeqiD() + 6) % 12] + "生年");
    ...
    private synchronized void findFestival() {
        ...
        Matcher m;
        for (int i=0; i<LunarCalendar.sFtv.length; i++) {
            m = LunarCalendar.sFreg.matcher(LunarCalendar.sFtv[i]);
            if (m.find()) {
        ...
        for (int i=0; i<LunarCalendar.lFtv.length; i++) {
            m = LunarCalendar.sFreg.matcher(LunarCalendar.lFtv[i]);
            if (m.find()) {
        ...
        // 月周节日
        int w, d;
        for (int i=0; i<LunarCalendar.wFtv.length; i++) {
            m = LunarCalendar.wFreg.matcher(LunarCalendar.wFtv[i]);
            if (m.find()) {
    ...
    private static int getLunarLeapMonth(int lunarYear) {
        ...
        int leapMonth = LunarCalendar.lunarInfo[lunarYear - 1900] & 0xf;
    ...
    private static int getLunarLeapDays(int lunarYear) {
        ...
        // 若该年没有闰月,返回0
        return LunarCalendar.getLunarLeapMonth(lunarYear) > 0 ? ((LunarCalendar.lunarInfo[lunarYear - 1899] & 0xf) == 0xf ? 30 : 29) : 0;
    }
    ...
    private static int getLunarYearDays(int lunarYear) {
        ...
        for (int i = 0x8000; i > 0x8; i >>= 1) {
            daysInLunarYear += ((LunarCalendar.lunarInfo[lunarYear - 1900] & i) != 0) ? 1
                    : 0;
        }
        // 加上闰月天数
    ...
    private static int getLunarMonthDays(int lunarYear, int lunarMonth) {
        // 数据表中,每个农历年用16bit来表示,
        // 前12bit分别表示12个月份的大小月,最后4bit表示闰月
        int daysInLunarMonth = ((LunarCalendar.lunarInfo[lunarYear - 1900] & (0x10000 >> lunarMonth)) != 0) ? 30
                : 29;
    ...
    private static int getSolarTermDay(int solarYear, int index) {
        ...
        l = l + LunarCalendar.UTC(1900,0,6,2,5,0);
        return LunarCalendar.getUTCDay(new Date(l));
    }
    ...
    private void getCyclicalData() {
        ...
        int term2 = LunarCalendar.getSolarTermDay(solarYear, 2); // 立春日期
        // 依节气调整二月分的年柱, 以立春为界
        ...
        int firstNode = LunarCalendar.getSolarTermDay(solarYear, solarMonth * 2); // 传回当月「节」为几日开始
        // 依节气月柱, 以「节」为界
    ...
    public String getTermString() {
        ...
        if (LunarCalendar.getSolarTermDay(solarYear, solarMonth * 2) == solarDay) {
            termString = LunarCalendar.solarTerm[solarMonth * 2];
        } else if (LunarCalendar.getSolarTermDay(solarYear, solarMonth * 2 + 1) == solarDay) {
            termString = LunarCalendar.solarTerm[solarMonth * 2 + 1];
        }
        return termString;
    }
    ...
    private static String getCyclicalString(int cyclicalNumber) {
        return LunarCalendar.Tianan[LunarCalendar.getTianan(cyclicalNumber)] + LunarCalendar.Deqi[LunarCalendar.getDeqi(cyclicalNumber)];
    }
    ...
    private static String getLunarDayString(int lunarDay) {
        ...
        if (lunarDay < 11) c1 = LunarCalendar.lunarString2[0];
        if (i2 == 0) c2 = LunarCalendar.lunarString2[1];
        return c1 + c2;
    }
