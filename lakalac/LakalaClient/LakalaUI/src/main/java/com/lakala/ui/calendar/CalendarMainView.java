 ...
 * months). Must be initialized after inflation with {@link #init(java.util.Date, java.util.Date)}
 ...
 *
 * usage
 * 1. 初始化最小日期与最大日期，minCal和maxCal,均为Calendar对象
 * 2. 初始化一个已经处于选择状态的日期selectedDate,为Date类型,此步骤可选
 * 3. 获取CalendarMainView实例，calendarMainView
 *          calendarMainView.init(minCal.getTime(), maxCal.getTime())
                            .inMode(CalendarMainView.SelectionMode.SINGLE) //日历模式，single为单选模式，即只能选择一个日期,具体可参考SelectionMode
                            .withSelectedDate(selectedDate); //初始化一个已经选择的日期(可选)
 *
 */
public class CalendarMainView extends ListView {
    ...
    public CalendarMainView(Context context, AttributeSet attrs) {
        ...
        setDivider(null);
        setDividerHeight(20);
        final int bg = getResources().getColor(R.color.l_white);
    ...
    /**
     * Both date parameters must be non-null and their {@link java.util.Date#getTime()}
...
//                smoothScrollToPosition(selectedIndex);
     ...
     * CalendarMainView is configured with: if you are in
    ...
    List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month,
        ...
        while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month
            ...
            for (int c = 0; c < 7; c++) {
                ...
                if (Holiday.holidays.containsKey(key)) {
                    LunarCalendar lunarCalendar = Holiday.holidays.get(key);
                    ...
                    monthCellDescriptor.setHolidayName(holidayName);
                }
     ...
     * <p/>
     * Important: set this before you call {@link #init(java.util.Date, java.util.Date)} methods. If
     ...
     * {@link #selectDate(java.util.Date)} this listener will not be notified.
     *
     ...
     * you call {@link #selectDate(java.util.Date)} this listener will not be notified.
     *
