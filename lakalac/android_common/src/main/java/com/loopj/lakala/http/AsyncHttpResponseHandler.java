...
/**
 * Used to intercept and handle the responses from requests made using {@link com.loopj.lakala.http.AsyncHttpClient}. The
...
public abstract class AsyncHttpResponseHandler implements ResponseHandlerInterface {
    ...
    public void setUseSynchronousMode(boolean value) {
        ...
        if (!value && Looper.myLooper() == null) {
            value = true;
            LogUtil.w(LOG_TAG, "Current thread has not called Looper.prepare(). Forcing synchronous mode.");
        }
     ...
     */
    public void onProgress(int bytesWritten, int totalSize) {
        LogUtil.v(LOG_TAG, String.format("Progress %d from %d (%2.0f%%)", bytesWritten, totalSize, (totalSize > 0) ? (bytesWritten * 1.0 / totalSize) * 100 : -1));
    }
            ...
            case PROGRESS_MESSAGE:
                ...
                if (response != null && response.length >= 2) {
                    try {
                        ...
                        LogUtil.e(LOG_TAG, "custom onProgress contains an error", t);
                    }
                } else {
                    LogUtil.e(LOG_TAG, "PROGRESS_MESSAGE didn't got enough params");
                }
                break;
            case RETRY_MESSAGE:
                response = (Object[]) message.obj;
                if (response != null && response.length == 1)
                    ...
                    LogUtil.e(LOG_TAG, "RETRY_MESSAGE didn't get enough params");
                break;
            case CANCEL_MESSAGE:
                onCancel();
                break;
