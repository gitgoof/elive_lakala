    ...
    protected void handleResponseBody(int statusCode,byte[] responseBody){
        ...
        switch (responseDataType){
            case RESPONSE_DATA_TYPE_JSON:
                ...
                this.resultCode = responseData.optString("retCode","");
                this.resultMessage = responseData.optString("retMsg","");

                //首先尝试将 _ReturnData 转换成 JSONObject，如果转换失败则尝试将其转换成 JSONArray，如果都转换失败则发送失败事件给 HttpRequest。
                try {
                    this.resultData = responseData.getJSONObject("retData");
                } catch (Exception e) {}

                if (this.resultData == null){
                    try {
                        this.resultData = responseData.getJSONArray("retData");
                    }catch (Exception e) {}
                }
