...
package com.lakala.core.scanner.zxing.wifi;
...
public final class WifiConfigManager extends AsyncTask<WifiParsedResult,Object,Object> {
  ...
  protected Object doInBackground(WifiParsedResult... args) {
    ...
    if (!wifiManager.isWifiEnabled()) {
      LogUtil.i(TAG, "Enabling wi-fi...");
      if (wifiManager.setWifiEnabled(true)) {
        LogUtil.i(TAG, "Wi-fi enabled");
      } else {
        LogUtil.w(TAG, "Wi-fi could not be enabled!");
        return null;
      }
      ...
      while (!wifiManager.isWifiEnabled()) {
        if (count >= 10) {
          LogUtil.i(TAG, "Took too long to enable wi-fi, quitting");
          return null;
        }
        LogUtil.i(TAG, "Still waiting for wi-fi to enable...");
        try {
          Thread.sleep(1000L);
    ...
    try {
      ...
      LogUtil.w(TAG, "Bad network type; see NetworkType values: " + networkTypeString);
      return null;
    }
  ...
  private static void updateNetwork(WifiManager wifiManager, WifiConfiguration config) {
    ...
    if (foundNetworkID != null) {
      LogUtil.i(TAG, "Removing old configuration for network " + config.SSID);
    ...
    if (networkId >= 0) {
      ...
      if (wifiManager.enableNetwork(networkId, true)) {
        LogUtil.i(TAG, "Associating to network " + config.SSID);
        wifiManager.saveConfiguration();
      } else {
        LogUtil.w(TAG, "Failed to enable network " + config.SSID);
      }
    } else {
      LogUtil.w(TAG, "Unable to add network " + config.SSID);
    }
  }
