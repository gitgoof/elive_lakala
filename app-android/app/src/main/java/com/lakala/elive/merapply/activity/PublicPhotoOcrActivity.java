...
public class PublicPhotoOcrActivity extends BaseActivity {
...
//    private boolean postBank = false;
    ...
    protected void setContentViewId() {
        setContentView(R.layout.activity_public_photo_ocr);
    }
...
//        imgBankPhoto.setOnClickListener(this);
    ...
    protected void bindData() {
        ...
        if (!TextUtils.isEmpty(applyId)) {//编辑状态下会接收到applyid
    ...
    public void onSuccess(int method, Object obj) {
        ...
        if (method == NetAPI.ACTION_MER_APPLY4) {//成功  回显照片
            ...
            switch (photoResq.getContent().fileType) {
                case ID_CARD_FRONT:
                    if(photoResq== null && photoResq.getContent().getFileContent()==null){
                        break;
                    }
                    postIdFront = true;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == OpenCameraUtil.newInstace().REQUEST_CODE_PICK_IMAGE && null != data) {//相册获取
            ...
            if(TextUtils.isEmpty(path)){
                Utils.showToast(PublicPhotoOcrActivity.this, "没有图片返回!");
                return;
            }
            final File file = new File(path);
            if(!file.exists()){
                Utils.showToast(PublicPhotoOcrActivity.this, "图片不存在!");
                return;
            }
            if(file.isDirectory()){
                Utils.showToast(PublicPhotoOcrActivity.this, "图片不存在!");
