...
import com.lakala.elive.merapply.activity.PrivatePhotoOcrActivity;
...
public class QCodePrivatePhotoOcrActivity extends BaseActivity {

    private ImageView imgIdPhoto, imgIdPhoto2, imgProtocolPhoto, imgProgress,
            imgProtocolPhoto2,
            imgProtocolPhoto1,
            imgBankPhoto;
    private TextView tvId, tvId2, tvProtocol,
            tvProtocol1,
            tvProtocol2,
            tvBank;
    ...
    private final int TYPE_BANK_CARD = 5;//银行卡正面
    ...
    private boolean bankFront = false;//银行卡判断是否上传成功
...
//    private boolean postXY = false;
    ...
    protected void setContentViewId() {
        setContentView(R.layout.activity_qcode_private_photo_ocr);
    }
    ...
    protected void bindView() {
        ...
        tvBank = findView(R.id.tv_bankcard);
        //身份证正面
        ...
        EliveApplication.setImageQCodeProgress(imgProgress, 1, EliveApplication.PRIVICEQCODE);
    ...
    protected void bindData() {
        ...
        if (!TextUtils.isEmpty(applyId) && qcodeComStatue != null && "1".equals(qcodeComStatue)) {//编辑状态
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_iv_back:
                showAlertDialog();
                break;
            case R.id.img_bank_photo://银行卡正面
...
//                     .putExtra(QCodePrivatePhotoOcrActivity.PHOTO_DISCERN_RESP, contentBeanNext)
        ...
        if (method == NetAPI.ACTION_MER_APPLY4) {//成功  回显照片
            ...
            switch (photoResq.getContent().fileType) {
                case BANK_CARD://银行卡正面
                    ...
                    setOcrEditResult(photoResq, tvBank);
                    break;
                case ID_CARD_FRONT:
                    postIdFront = true;
...
//                    break;
            ...
            switch (method) {
                case TYPE_BANK_CARD://银行卡
                    if (null == photoDiscernContent.getBankCardInfo() || TextUtils.isEmpty(photoDiscernContent.getBankCardInfo().getCard_number())) {
...
//                && postXY
    ...
    public void onError(int method, String statusCode) {
        ...
        if (method == NetAPI.ACTION_MER_APPLY_DETAILS) {
            ...
            switch (method) {
                case TYPE_BANK_CARD://银行卡正面
                    bankFront = false;
                    tvBank.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_wrong, 0, 0, 0);
                    imgBankPhoto.setImageResource(R.drawable.selector_photo);
                    Utils.showToast(this, "银行卡正面上传失败");
                    break;
                case TYPE_ID_CARD_FRONT:
                    postIdFront = false;
                    tvId.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_wrong, 0, 0, 0);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == OpenCameraUtil.newInstace().REQUEST_CODE_PICK_IMAGE && null != data) {//相册获取
            ...
            if(TextUtils.isEmpty(path)){
                Utils.showToast(QCodePrivatePhotoOcrActivity.this, "没有图片返回!");
                return;
            }
            final File file = new File(path);
            if(!file.exists()){
                Utils.showToast(QCodePrivatePhotoOcrActivity.this, "图片不存在!");
                return;
            }
            if(file.isDirectory()){
                Utils.showToast(QCodePrivatePhotoOcrActivity.this, "图片不存在!");
    ...
    private void setImgeContent(String path, MerAttachFile merAttachFile) {
        switch (merAttachFile.getFileType()) {
            ...
            case ID_CARD_BEHIND://身份证反面
                ...
                break;
            case XY://商户协议
                if (merAttachFile.getComments().equals("1")) {
                    instance.loadImage(path, imgProtocolPhoto);
                } else if (merAttachFile.getComments().equals("2")) {
                    instance.loadImage(path, imgProtocolPhoto1);
                } else if (merAttachFile.getComments().equals("3")) {
