...
public class PhotoFileFactory {
    ...
    private final String OTHERS = "OTHERS";//机器照
    ...
    public PhotoFileFactory(Context context) {
...
//        merAttachFile.setMerchantId(merchantId);

        if (XY.equals(photoType) || KH.equals(photoType) || SUP_PROOF.equals(photoType)//商户协议,开户许可证,附件,营业场所,门头照,机器照都不要需要ocr
                || SHOPINNER.equals(photoType) || MT.equals(photoType)
                || SYT.equals(photoType) || OTHERS.equals(photoType)) {
        ...
        if (XY.equals(this.photoType) || SUP_PROOF.equals(photoType)
                || MT.equals(photoType)
                || OTHERS.equals(photoType)) {
