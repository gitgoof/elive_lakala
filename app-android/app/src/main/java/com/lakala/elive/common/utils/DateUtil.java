...
public class DateUtil {
	...
	public static Date parseString(String dateStr, String dateFormat, String locale) {
		...
		if (StringUtil.isNotNullAndBlank(dateFormat)) {
			...
			if (hasBlank) {
				...
				if (dateStr.indexOf(":") != -1) {
					...
					splitCount = StringUtil.matchIndexCount(dateStr, "/", 0);
					if (splitCount == 2) {
		...
		DateFormat df = (StringUtil.isNullOrBlank(locale)) ? new SimpleDateFormat(realDF)
	...
	public static String formatDate(Object dt, String fmt, String locale) {
		...
		DateFormat df = StringUtil.isNullOrBlank(locale) ? new SimpleDateFormat(fmt)
	...
	public static void main(String[] args) {
		System.err.println(DateUtil.formatDate("2014年12月5号", "yyyy-MM-dd"));
	}
